...
int main() {
    ...
    printf("--- Earliest-Deadline-First (EDF) Simulation ---\n");
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        tasks[i].absolute_deadline = 999999; // Initialize to infinity
    }
    ...
    for (int time = 0; time < sim_time; time++) {
        ...
        // 2. Find the highest priority ready task (earliest deadline)
        int selected_task = -1;
        int earliest_deadline = 999999;

        for (int i = 0; i < num_tasks; i++) {
            if (tasks[i].remaining_time > 0 && tasks[i].absolute_deadline < earliest_deadline) {
        ...
        if (selected_task != -1) {
            if (current_task != selected_task) {
                printf("[Time %3d] CPU switched to Task %d (Deadline: %d)\n", 
                       time, tasks[selected_task].id, tasks[selected_task].absolute_deadline);
